...
- Do NOT write perfectly structured textbook code. Be slightly messy like a real person:
  ...
  - Use inline expressions where a human would (e.g., "return sum(1 for x in arr if x > 0)" instead of a loop)
- Avoid the most common LeetCode/GeeksForGeeks solution patterns. Use a slightly different approach or variable structure than what's on the first page of Google results.
- Do NOT use overly elegant or clever one-liners that scream "expert/AI". Write it like a solid mid-level developer would.
- Keep the code compact — no unnecessary blank lines between statements. Humans under pressure write dense code.
- If using Python, prefer simple constructs. Don't use functools, itertools, or fancy stdlib unless absolutely necessary.
- The code should look like it was typed quickly by a competent human, not generated by AI.
